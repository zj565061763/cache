...
本库不提供跨进程互斥：多个进程同时写入时不保证执行顺序和结果，`CacheKtx.edit`、`SingleCacheKtx.update` 的读改写也只在当前进程内原子化。需要跨进程并发访问时，调用方必须通过 IPC、文件锁或其他方式自行串行化。Flow 和内存缓存通过 `FileObserver` 异步感知其他进程的修改，不提供即时一致性。
